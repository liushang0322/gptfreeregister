...
def normalize_account_run_snapshot_record(record):
    ...
    if not email_addr or final_status not in {"success", "failed", "stopped", "running"}:
        return None
...
def normalize_account_run_snapshot_payload(payload):
    ...
    for item in raw_records:
        ...
        if normalized and normalized.get("password"):
            keys = {
                str(normalized.get("recordId") or "").strip().lower(),
                str(normalized.get("email") or "").strip().lower(),
            }
            for key in keys:
                if key:
                    password_by_key[key] = normalized["password"]
    ...
    for item in raw_records:
        ...
        if normalized:
            ...
            existing_password = next((password_by_key.get(key) for key in keys if key and password_by_key.get(key)), "")
            if not normalized.get("password") and existing_password:
                normalized["password"] = existing_password
