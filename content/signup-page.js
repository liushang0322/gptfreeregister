...
if (document.documentElement.getAttribute(SIGNUP_PAGE_LISTENER_SENTINEL) !== '1') {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (
      ...
      || message.type === 'RESUBMIT_STEP5_PROFILE'
...
async function handleCommand(message) {
  switch (message.type) {
    ...
    case 'RESUBMIT_STEP5_PROFILE':
      return await resubmitStep5Profile(message.payload);
...
async function resubmitStep5Profile(payload = {}) {
  if (!isStep5ProfileStillVisible()) {
    return getStep5SubmitState();
  }

  const submitButton = await waitForStep5SubmitButton(payload?.buttonTimeoutMs ?? 5000);
  ...
  log('步骤 5：资料页仍可见，正在重新点击“完成帐户创建”并等待结果...', 'warn');
  await humanPause(350, 900);
  await performOperationWithDelay({ stepKey: 'fill-profile', kind: 'submit', label: 'resubmit-profile' }, async () => {
    simulateClick(submitButton);
  });

  const outcome = await waitForStep5SubmitOutcome({
    timeoutMs: payload?.timeoutMs ?? 45000,
    maxAuthRetryRecoveries: payload?.maxAuthRetryRecoveries ?? 2,
    maxSubmitClicks: payload?.maxSubmitClicks ?? 2,
  });
  return {
    ...getStep5SubmitState(),
    resubmitted: true,
    successState: outcome?.state || getStep5SubmitState().successState || '',
