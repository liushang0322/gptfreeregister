...
- `background/contribution-oauth.js`：贡献模式的 OAuth 流程模块。当前公开仓库构建默认不再内置第三方贡献接口地址；如需启用，需要你自行配置对应服务端地址与回调链路。
...
- `background/steps/create-plus-checkout.js`：Plus 模式第 6 步实现，负责在已登录 ChatGPT 页面创建 PayPal / GoPay checkout session，或在 GPC 模式下读取 accessToken 并通过 `https://your-gpc-helper-domain.example/api/gp/tasks` 创建队列任务，记录 checkout / GPC task 运行态。
...
- `sidepanel/contribution-content-update-service.js`：侧边栏贡献内容更新服务，负责拉取 `<your-contribution-service>/api/content-summary`、缓存公开公告/教程摘要，并输出可用于提示展示的 `promptVersion` 与最新更新时间。
