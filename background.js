...
const NORMAL_SUB2API_SESSION_STEP_DEFINITIONS = self.MultiPageStepDefinitions?.getSteps?.({
  activeFlowId: DEFAULT_ACTIVE_FLOW_ID,
  plusModeEnabled: false,
  plusAccountAccessStrategy: PLUS_ACCOUNT_ACCESS_STRATEGY_SUB2API_CODEX_SESSION,
}) || NORMAL_STEP_DEFINITIONS;
const NORMAL_CPA_SESSION_STEP_DEFINITIONS = self.MultiPageStepDefinitions?.getSteps?.({
  activeFlowId: DEFAULT_ACTIVE_FLOW_ID,
  plusModeEnabled: false,
...
const ALL_STEP_DEFINITIONS = self.MultiPageStepDefinitions?.getAllSteps?.({
  ...
  ...NORMAL_STEP_DEFINITIONS,
  ...NORMAL_SUB2API_SESSION_STEP_DEFINITIONS,
  ...NORMAL_CPA_SESSION_STEP_DEFINITIONS,
  ...NORMAL_PHONE_STEP_DEFINITIONS,
...
const HOTMAIL_LOCAL_HELPER_TIMEOUT_MS = 30000;
const HOTMAIL_ACCOUNT_VERIFY_TIMEOUT_MS = 25000;
...
function getStepDefinitionsForState(state = {}) {
  ...
  if (!isPlusModeState(state)) {
    ...
    if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_SUB2API_CODEX_SESSION) {
      return NORMAL_SUB2API_SESSION_STEP_DEFINITIONS;
    }
    if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION) {
      return NORMAL_CPA_SESSION_STEP_DEFINITIONS;
    }
...
const normalSub2ApiSessionStepRegistry = buildStepRegistry(NORMAL_SUB2API_SESSION_STEP_DEFINITIONS);
const normalCpaSessionStepRegistry = buildStepRegistry(NORMAL_CPA_SESSION_STEP_DEFINITIONS);
...
function getStepRegistryForState(state = {}) {
  ...
  if (!isPlusModeState(state)) {
    ...
    if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_SUB2API_CODEX_SESSION) {
      return normalSub2ApiSessionStepRegistry;
    }
    if (plusAccountAccessStrategy === PLUS_ACCOUNT_ACCESS_STRATEGY_CPA_CODEX_SESSION) {
      return normalCpaSessionStepRegistry;
    }
...
async function resubmitStep5ProfileOnTab(options = {}) {
  const result = await sendToContentScriptResilient(
    'signup-page',
    {
      type: 'RESUBMIT_STEP5_PROFILE',
      source: 'background',
      payload: {
        timeoutMs: options.timeoutMs ?? 45000,
        maxAuthRetryRecoveries: options.maxAuthRetryRecoveries ?? 2,
        maxSubmitClicks: options.maxSubmitClicks ?? 2,
      },
    },
    {
      timeoutMs: options.timeoutMs ?? 50000,
      retryDelayMs: options.retryDelayMs ?? 600,
      responseTimeoutMs: options.responseTimeoutMs ?? (options.timeoutMs ?? 50000),
      logMessage: options.logMessage || '步骤 5：资料页仍停留，正在重新提交并等待页面结果...',
      logStep: 5,
      logStepKey: options.logStepKey || 'fill-profile',
    }
  );
...
async function validateStep5PostCompletion(tabId, completionPayload = {}) {
  ...
  const maxProfileResubmits = Math.max(1, Number(completionPayload?.maxProfileResubmits) || 2);
  let profileResubmitCount = 0;
  ...
  while (true) {
    ...
    if (pageState.profileVisible) {
      if (profileResubmitCount >= maxProfileResubmits) {
        throw new Error(`步骤 5：资料提交完成信号已收到，但页面仍停留在资料页，已重提 ${profileResubmitCount}/${maxProfileResubmits} 次。URL: ${pageState.url || currentUrl || 'unknown'}`);
      }
      profileResubmitCount += 1;
      await addLog(`步骤 5：资料提交完成信号后仍停留在资料页，正在重新提交（${profileResubmitCount}/${maxProfileResubmits}）...`, 'warn', {
        step: 5,
        stepKey: 'fill-profile',
      });
      const resubmitState = await resubmitStep5ProfileOnTab({
        timeoutMs: 50000,
        retryDelayMs: 600,
        logMessage: '步骤 5：资料页重新提交后正在等待页面跳转或恢复...',
      });
      if (resubmitState.successState === 'logged_in_home' || resubmitState.successState === 'oauth_consent' || resubmitState.successState === 'add_phone' || resubmitState.successState === 'left_profile') {
