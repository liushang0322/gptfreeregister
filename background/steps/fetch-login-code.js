(function attachBackgroundStep8(root, factory) {
  ...
  function createStep8Executor(deps = {}) {
    ...
    async function waitForManualPostLoginPhoneContinuation(state, visibleStep) {
      const startedAt = Date.now();
      const timeoutMs = typeof getOAuthFlowStepTimeoutMs === 'function'
        ? await getOAuthFlowStepTimeoutMs(15 * 60 * 1000, {
          step: visibleStep,
          actionLabel: '等待手动完成手机号验证',
          oauthUrl: state?.oauthUrl || '',
        })
        : 15 * 60 * 1000;
      let lastState = '';

      await addLog(`步骤 ${visibleStep}：检测到手机号验证页，接码未开启；请在当前认证页手动填写手机号/验证码，完成后将自动继续。`, 'warn', {
        step: visibleStep,
      ...
      while (Date.now() - startedAt < timeoutMs) {
        ...
        const pageState = await getLoginAuthStateFromContent(visibleStep, {
          timeoutMs: 10000,
          logMessage: `步骤 ${visibleStep}：等待你手动完成手机号验证后进入下一页...`,
          logStepKey: activeFetchLoginCodeStepKey,
        });
        const currentState = pageState?.state || 'unknown';
        if (currentState !== lastState) {
          lastState = currentState;
          await addLog(`步骤 ${visibleStep}：当前认证页状态：${currentState}。`, 'info', {
            step: visibleStep,
            stepKey: activeFetchLoginCodeStepKey,
          });
        }
        ...
        if (currentState === 'verification_page') {
          await pollEmailVerificationCode(state, pageState, visibleStep, {
            stickyLastResendAt: Number(state?.loginVerificationRequestedAt) || 0,
          });
          return;
        }
        if (currentState !== 'add_phone_page' && currentState !== 'phone_verification_page' && currentState !== 'unknown') {
          throw new Error(`步骤 ${visibleStep}：手动手机号验证后进入了不支持的页面：${currentState}。URL: ${pageState?.url || ''}`.trim());
        }

        await sleepWithStop(2000);
      }

      throw new Error(`步骤 ${visibleStep}：等待手动完成手机号验证超时，请确认认证页是否仍停留在手机号流程。`);
    }
