(function attachBackgroundSub2ApiApi(root, factory) {
  ...
  function createSub2ApiApi(deps = {}) {
    ...
    function createSub2ApiRequestError(payload, responseStatus = 500, path = '') {
      const error = new Error(getSub2ApiErrorMessage(payload, responseStatus, path));
    ...
    function isSub2ApiNotFoundError(error, path = '') {
      const message = normalizeString(error?.message);
      const errorPath = normalizeString(error?.path);
      return Number(error?.status) === 404
        || (/HTTP\s*404/i.test(message) && (!path || message.includes(path)))
        || (path && errorPath === path && /404/.test(message));
    }
    ...
    async function requestJson(origin, path, options = {}) {
      ...
      try {
        ...
        if (payload && typeof payload === 'object' && Object.prototype.hasOwnProperty.call(payload, 'code')) {
          ...
          throw createSub2ApiRequestError(payload, response.status, path);
        }
    ...
    function getCodexOpenAiAuthSection(claims = {}) {
      const auth = claims?.['https://api.openai.com/auth'];
      return auth && typeof auth === 'object' && !Array.isArray(auth) ? auth : {};
    }

    function getCodexOpenAiProfileSection(claims = {}) {
      const profile = claims?.['https://api.openai.com/profile'];
      return profile && typeof profile === 'object' && !Array.isArray(profile) ? profile : {};
    }

    function resolveCodexTokenExpiresAtFromClaims(claims = {}) {
      const expiresAt = Math.floor(Number(claims?.exp) || 0);
      return expiresAt > 0 ? expiresAt : null;
    }

    function normalizeCodexUnixSeconds(value) {
      if (value === undefined || value === null || value === '') {
        return 0;
      }
    ...
    function resolveCodexOrganizationId(sessionObject = {}, authClaims = {}) {
      const organizations = Array.isArray(authClaims.organizations) ? authClaims.organizations : [];
      const defaultOrganization = organizations.find((org) => Boolean(org?.is_default || org?.isDefault || org?.default));
      return normalizeString(
        sessionObject.account?.organization_id
        || sessionObject.account?.organizationId
        || sessionObject.organization_id
        || sessionObject.organizationId
        || sessionObject.org_id
        || sessionObject.orgId
        || sessionObject.credentials?.organization_id
        || sessionObject.credentials?.organizationId
        || sessionObject.providerSpecificData?.organization_id
        || sessionObject.providerSpecificData?.organizationId
    ...
    function resolveCodexSessionExpiresAt(session) {
      const sessionObject = normalizeCodexSessionObject(session);
    ...
    function resolveCodexSessionImportExpiresAt(session, accessToken = '') {
      const sessionObject = normalizeCodexSessionObject(session);
      const normalizedAccessToken = normalizeString(
        accessToken
        || sessionObject?.accessToken
        || sessionObject?.access_token
        || sessionObject?.token?.accessToken
        || sessionObject?.token?.access_token
        || sessionObject?.credentials?.accessToken
        || sessionObject?.credentials?.access_token
      );
      const claims = parseCodexAccessTokenClaims(normalizedAccessToken) || {};
      return resolveCodexTokenExpiresAtFromClaims(claims) || resolveCodexSessionExpiresAt(sessionObject);
    }
    ...
    function normalizeAccountDataImportResult(result) {
      const errors = (Array.isArray(result?.errors) ? result.errors : [])
        .map((item, index) => ({
          index: Number(item?.index) || index + 1,
          name: normalizeString(item?.name || item?.proxy_key),
          message: normalizeString(item?.message),
        }))
        .filter((item) => item.message);
      return {
        created: Math.max(0, Number(result?.account_created) || 0),
        failed: Math.max(0, Number(result?.account_failed) || 0),
        proxyCreated: Math.max(0, Number(result?.proxy_created) || 0),
        proxyReused: Math.max(0, Number(result?.proxy_reused) || 0),
        proxyFailed: Math.max(0, Number(result?.proxy_failed) || 0),
        errors,
      };
    }
    ...
    function getAccountDataImportFailureMessage(result) {
      const normalized = normalizeAccountDataImportResult(result);
      return normalized.errors.map((item) => item.message).find(Boolean)
        || `SUB2API 数据文件导入失败：账号新建 ${normalized.created}，账号失败 ${normalized.failed}`;
    }
    ...
    function buildOpenAiCredentialsFromCodexSession(session, accessToken = '') {
      const sessionObject = normalizeCodexSessionObject(session) || {};
      const normalizedAccessToken = normalizeString(
        accessToken
        || sessionObject.accessToken
      ...
      const claims = parseCodexAccessTokenClaims(normalizedAccessToken) || {};
      const authClaims = getCodexOpenAiAuthSection(claims);
      const profileClaims = getCodexOpenAiProfileSection(claims);
      const credentials = {
        access_token: normalizedAccessToken,
      };
      const expiresAt = resolveCodexSessionImportExpiresAt(sessionObject, normalizedAccessToken);
      const email = normalizeEmailValue(sessionObject?.user?.email)
        || normalizeEmailValue(sessionObject?.email)
        || normalizeEmailValue(sessionObject?.credentials?.email)
        || normalizeEmailValue(sessionObject?.providerSpecificData?.email)
        || normalizeEmailValue(profileClaims?.email)
        || normalizeEmailValue(claims?.email);
      const refreshToken = normalizeString(
        sessionObject.refreshToken
        || sessionObject.refresh_token
        || sessionObject.token?.refreshToken
        || sessionObject.token?.refresh_token
        || sessionObject.credentials?.refreshToken
        || sessionObject.credentials?.refresh_token
      );
      const idToken = normalizeString(
        sessionObject.idToken
        || sessionObject.id_token
        || sessionObject.token?.idToken
        || sessionObject.token?.id_token
        || sessionObject.credentials?.idToken
        || sessionObject.credentials?.id_token
      );
      const chatGptAccountId = normalizeString(
        sessionObject.account?.id
        || sessionObject.account_id
        || sessionObject.chatgptAccountId
        || sessionObject.chatgpt_account_id
        || sessionObject.credentials?.chatgptAccountId
        || sessionObject.credentials?.chatgpt_account_id
        || sessionObject.providerSpecificData?.chatgptAccountId
        || sessionObject.providerSpecificData?.chatgpt_account_id
        || authClaims.chatgpt_account_id
      );
      const chatGptUserId = normalizeString(
        sessionObject.user?.id
        || sessionObject.user_id
        || sessionObject.chatgptUserId
        || sessionObject.chatgpt_user_id
        || sessionObject.credentials?.chatgptUserId
        || sessionObject.credentials?.chatgpt_user_id
        || sessionObject.providerSpecificData?.chatgptUserId
        || sessionObject.providerSpecificData?.chatgpt_user_id
      ...
      const planType = normalizeString(
        ...
        || sessionObject.credentials?.planType
        || sessionObject.credentials?.plan_type
        || sessionObject.providerSpecificData?.chatgptPlanType
        || sessionObject.providerSpecificData?.chatgpt_plan_type
        || authClaims.chatgpt_plan_type
      );
      const organizationId = resolveCodexOrganizationId(sessionObject, authClaims);
      ...
      if (refreshToken) {
        credentials.client_id = 'app_EMoamEEZ73f0CkXaXp7hrann';
      }

      return credentials;
    }

    function buildOpenAiExtraFromCodexSession(session) {
      const sessionObject = normalizeCodexSessionObject(session) || {};
      const extra = {};
      const email = normalizeEmailValue(sessionObject?.user?.email)
        || normalizeEmailValue(sessionObject?.email);
      const name = normalizeString(sessionObject?.user?.name || sessionObject?.name);
      const sessionExpiresAt = resolveCodexSessionExpiresAt(sessionObject);
      if (email) {
        extra.email = email;
      }
      if (name) {
      ...
      if (sessionExpiresAt) {
        extra.session_expires_at = new Date(sessionExpiresAt * 1000).toISOString();
      }
      return Object.keys(extra).length ? extra : undefined;
    }

    function buildCodexSessionDirectCreatePayload({
      state = {},
      session = null,
      accessToken = '',
      groups = [],
      proxyId = null,
      accountPriority = DEFAULT_PRIORITY,
      preferredAccountName = '',
    } = {}) {
      const credentials = buildOpenAiCredentialsFromCodexSession(session, accessToken);
      const extra = buildOpenAiExtraFromCodexSession(session);
      ...
      if (expiresAt) {
        createPayload.expires_at = expiresAt;
      }
      if (proxyId) {
    ...
    function buildCodexSessionAccountDataPayload(options = {}) {
      ...
      return {
        ...
        accounts: [{
          ...
          ...(autoPauseOnExpired === undefined ? {} : { auto_pause_on_expired: Boolean(autoPauseOnExpired) }),
        }],
      };
    }

    async function importCodexSessionViaAccountData(origin, token, options = {}) {
      const dataPayload = buildCodexSessionAccountDataPayload(options);
    ...
    async function importCurrentChatGptSession(state = {}, options = {}) {
      ...
      const importExpiresAt = resolveCodexSessionImportExpiresAt(session, accessToken);
      ...
      const codexSessionImportPath = '/api/v1/admin/accounts/import/codex-session';
      try {
        importResult = normalizeCodexSessionImportResult(await requestJson(origin, codexSessionImportPath, {
          method: 'POST',
          token,
          timeoutMs: options.importTimeoutMs || options.timeoutMs,
          body: importPayload,
        ...
        if (!isSub2ApiNotFoundError(error, codexSessionImportPath)) {
          throw error;
        }

        await logWithOptions(`${logLabel}：当前 SUB2API 实例未提供会话批量导入接口，改用账号创建接口。`, 'warn', options);
        ...
        try {
          const createPayload = buildCodexSessionDirectCreatePayload(fallbackOptions);
          const createdAccount = await requestJson(origin, '/api/v1/admin/accounts', {
            method: 'POST',
            token,
            timeoutMs: options.importTimeoutMs || options.timeoutMs,
          ...
          const verifiedStatus = `SUB2API 已创建账号 #${createdAccount?.id || 'unknown'}（accessToken 导入，可能无法自动续期）`;
          await logWithOptions(verifiedStatus, 'ok', options);
          return {
            verifiedStatus,
            sub2apiImportTotal: 1,
            sub2apiImportCreated: 1,
            sub2apiImportUpdated: 0,
            sub2apiImportSkipped: 0,
            sub2apiImportFailed: 0,
            sub2apiFallbackMode: 'accounts-create',
            sub2apiFallbackCreatedAccountId: createdAccount?.id || null,
          };
        } catch (createError) {
          await logWithOptions(`${logLabel}：账号创建接口失败，最后尝试后台“数据文件 JSON 导入”接口。原因：${getSub2ApiErrorMessage(createError?.payload, createError?.status || 500, createError?.path || '/api/v1/admin/accounts')}`, 'warn', options);
          const dataImportResult = await importCodexSessionViaAccountData(origin, token, fallbackOptions);
          const verifiedStatus = `SUB2API 数据文件导入完成：新建账号 ${dataImportResult.created}，代理新建 ${dataImportResult.proxyCreated}，代理复用 ${dataImportResult.proxyReused}`;
          await logWithOptions(verifiedStatus, 'ok', options);
          return {
            verifiedStatus,
