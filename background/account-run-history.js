(function attachBackgroundAccountRunHistory(root, factory) {
  ...
  function createAccountRunHistoryHelpers(deps = {}) {
    ...
    function resolvePasswordFromAccounts(state = {}, identity = {}) {
      const accounts = Array.isArray(state.accounts) ? state.accounts : [];
      if (!accounts.length) {
        return '';
      }

      const targetRecordId = buildRecordId(identity.accountIdentifier, identity.accountIdentifierType);
      const targetEmail = String(identity.email || '').trim().toLowerCase();
      const targetPhone = normalizePhoneRecordKey(identity.phoneNumber);
      ...
      for (let index = accounts.length - 1; index >= 0; index -= 1) {
        ...
        const accountIdentity = resolveRecordIdentity(account);
        const accountRecordId = buildRecordId(accountIdentity.accountIdentifier, accountIdentity.accountIdentifierType);
        const accountEmail = String(accountIdentity.email || '').trim().toLowerCase();
        const accountPhone = normalizePhoneRecordKey(accountIdentity.phoneNumber);
        if ((targetRecordId && accountRecordId === targetRecordId)
          || (targetEmail && accountEmail === targetEmail)
          || (targetPhone && accountPhone === targetPhone)) {
          return password;
        }
      }
    ...
    function buildAccountRunHistoryRecord(state = {}, status = '', reason = '') {
      ...
      const password = String(state.password || state.customPassword || '').trim()
        || resolvePasswordFromAccounts(state, identity);
    ...
    function upsertAccountRunHistoryRecord(history, record) {
      ...
      const mergedRecord = {
        ...record,
        password: String(record.password || '').trim() || String(existingRecord?.password || '').trim(),
      };
