(function attachStepDefinitions(root, factory) {
  ...
  function createSub2ApiSessionImportTail(startId, startOrder, signupMethod = SIGNUP_METHOD_EMAIL, options = {}) {
    const authTail = createOpenAiAuthTail(startId, startOrder, signupMethod, options);
    return authTail.map((step) => (
      step.key === 'platform-verify'
        ? {
          ...step,
          title: '导入 Codex JSON 到 SUB2API',
  ...
  function resolveSessionImportTail(options = {}, signupMethod = SIGNUP_METHOD_EMAIL) {
    if (isPlusModeEnabled(options)) {
      return resolvePlusSessionImportTail(options, signupMethod);
    }
    return resolvePlusSessionImportTail(options, signupMethod);
  }
  ...
  const NORMAL_STEP_DEFINITIONS = createOpenAiSteps(NORMAL_REGISTRATION_PREFIX_STEP_DEFINITIONS, 7, 70, SIGNUP_METHOD_EMAIL);
  const NORMAL_SUB2API_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    NORMAL_REGISTRATION_PREFIX_STEP_DEFINITIONS,
    7,
    70,
    SIGNUP_METHOD_EMAIL,
    { plusAccountAccessStrategy: PLUS_ACCOUNT_ACCESS_STRATEGY_SUB2API_CODEX_SESSION }
  );
  const NORMAL_CPA_SESSION_STEP_DEFINITIONS = createOpenAiSteps(
    NORMAL_REGISTRATION_PREFIX_STEP_DEFINITIONS,
    7,
    70,
    SIGNUP_METHOD_EMAIL,
  ...
  const NORMAL_PHONE_STEP_DEFINITIONS = createOpenAiSteps(NORMAL_REGISTRATION_PREFIX_STEP_DEFINITIONS, 7, 70, SIGNUP_METHOD_PHONE);
  const NORMAL_PHONE_BOUND_EMAIL_RELOGIN_STEP_DEFINITIONS = createOpenAiSteps(NORMAL_REGISTRATION_PREFIX_STEP_DEFINITIONS, 7, 70, SIGNUP_METHOD_PHONE, { phoneSignupReloginAfterBindEmailEnabled: true });
